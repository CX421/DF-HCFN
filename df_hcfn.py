...
class BlockCrossAttention3D(nn.Module):
    def __init__(self, image_channels, text_dim=512, attn_dim=64, block_size=8):
        super().__init__()
        ...
        self.to_q = nn.Sequential(
            nn.Conv3d(image_channels, attn_dim, 1),
            nn.GroupNorm(8, attn_dim),
            nn.GELU()
        )
        ...
        self.gate = nn.Sequential(
            nn.Conv3d(attn_dim, 1, 1),
            nn.Sigmoid()
        )

        # 输出投影
        self.to_out = nn.Sequential(
            nn.Conv3d(attn_dim, image_channels, 1),
            nn.GroupNorm(8, image_channels)
        )

        self.scale = attn_dim ** -0.5

    def _block_process(self, x):
        """将3D特征分块处理"""
        B, C, D, H, W = x.shape
        assert D % self.block_size == 0 and H % self.block_size == 0 and W % self.block_size == 0, \
            f"空间维度必须能被block_size整除，当前尺寸({D},{H},{W})，block_size={self.block_size}"

        # 分块操作
        x = einops.rearrange(
            x,
            'b c (d_blk d) (h_blk h) (w_blk w) -> b (d_blk h_blk w_blk) c d h w',
            d=self.block_size,
            h=self.block_size,
            w=self.block_size
        )
    ...
    def _block_merge(self, x, original_shape):
        """将分块后的特征合并回原始形状"""
        D, H, W = original_shape  # 正确解包3个值

        x = einops.rearrange(
            x,
            'b (d_blk h_blk w_blk) c d h w -> b c (d_blk d) (h_blk h) (w_blk w)',
            d_blk=D // self.block_size,
            h_blk=H // self.block_size,
            w_blk=W // self.block_size
        )
        return x
    ...
    def forward(self, x, text1, text2):
        ...
        # 投影查询特征
        q = self.to_q(x)  # [B, A, D, H, W]

        # 分块处理
        q_blocks = self._block_process(q)  # [B, N_blk, A, blk_d, blk_h, blk_w]
        B, N_blk, A = q_blocks.shape[:3]

        # 文本特征融合
        k = self.text_proj['k'](text1 + text2)  # [B, A]
        v = self.text_proj['v'](text1 * text2)  # [B, A]

        # 空间维度展开
        q_blocks = einops.rearrange(q_blocks, 'b n a d h w -> b n (d h w) a')

        # 注意力计算（每个块独立计算）
        attn = torch.einsum('b n l a, b a -> b n l', q_blocks, k) * self.scale
        attn = attn.softmax(dim=-1)

        # 特征融合
        out_blocks = torch.einsum('b n l, b a -> b n l a', attn, v)
        out_blocks = einops.rearrange(out_blocks, 'b n (d h w) a -> b n a d h w',
                                      d=self.block_size, h=self.block_size, w=self.block_size)

        out = self._block_merge(out_blocks, original_shape)  # [B, A, D, H, W]

        # 应用门控机制（不改变通道数）
        gate = self.gate(out)  # [B, 1, D, H, W]
        out = out * gate  # [B, A, D, H, W]

        # 关键修复：将通道数转回原始输入维度
        out = self.to_out(out)  # [B, C, D, H, W]

        return identity + out  # 残差连接


class MaskModal(nn.Module):
    def __init__(self, embed_dim=512, text_dim=512, num_heads=8, device=None):
        super().__init__()
        # 优化点1: 自动检测设备
        ...
        if self.training:
            ...
            # 预计算文本特征并保存
            self.register_buffer('clip_text_features', self._precompute_text_features(clip_model, processor))
        ...
        self.cross_attns = nn.ModuleList([
            BlockCrossAttention3D(8, attn_dim=32, block_size=16),
            BlockCrossAttention3D(24, attn_dim=64, block_size=8),
            BlockCrossAttention3D(48, attn_dim=128, block_size=4),
            BlockCrossAttention3D(96, attn_dim=256, block_size=2),
            BlockCrossAttention3D(192, attn_dim=384, block_size=1)
        ])

        # 优化点5: 参数化文本投影
        self.text_projections = nn.ParameterList([
            nn.Parameter(torch.empty(embed_dim, text_dim)) for _ in range(4)
        ])
        self._init_parameters()

        # 优化点6: 统一的可学习文本嵌入
        self.learnable_embeds = nn.Embedding(4, embed_dim)

        # 优化点7: 共享的Transformer编码器
        ...
        self.text_encoder = nn.Sequential(
            nn.Linear(embed_dim, embed_dim),  # 融合CLIP特征和可学习嵌入
            nn.GELU(),
            nn.Linear(embed_dim, text_dim)  # 投影到目标维度
        )

        self.to(self.device)

    def _init_parameters(self):
        for proj in self.text_projections:
            nn.init.xavier_uniform_(proj)

    def _precompute_text_features(self, clip_model, processor):
        prompts = [
            'There is a FLAIR modality in this MRI image',
            'There is a T1CE modality in this MRI image',
            'There is a T1 modality in this MRI image',
            'There is a T2 modality in this MRI image'
        ]
        features = []
        for text in prompts:
            ...
            with torch.no_grad():
                features.append(clip_model.get_text_features(**inputs))
        return torch.stack(features, dim=0)

    # def _process_text_features(self, idx):
    #     # 优化点8: 批量处理文本特征
    #     clip_feature = self.clip_text_features[idx].expand(len(self.text_projections), -1)
    #     learned = self.learnable_embeds.weight[idx]
    #     combined = self.text_encoder(clip_feature + learned)
    #     return torch.matmul(combined, self.text_projections[idx])
    def _process_text_features(self, idx, batch_size):
        """根据模态索引生成对应文本特征"""
        # 获取当前模态的CLIP特征 [1, text_dim] -> [B, text_dim]
        clip_feature = self.clip_text_features[idx].expand(batch_size, -1)

        # 获取可学习嵌入 [embed_dim] -> [B, embed_dim]
        learned = self.learnable_embeds(torch.tensor([idx], device=self.device)).expand(batch_size, -1)

        # 特征融合与编码
        combined = self.text_encoder((clip_feature + learned).unsqueeze(1))

        # 模态专属投影 [B, text_dim]
        return torch.matmul(combined.squeeze(1), self.text_projections[idx])
    ...
    def forward(self, x, mask):
        ...
        # 优化点9: 向量化模态处理
        split_x = [x[:, i] for i in range(K)]  # 拆分为各模态
        # print(f"split_x shape: {[i.shape for i in split_x]}")
        processed = []

        for idx in range(4):
            # print(f"Processing modality {idx}")
            if mask[:, idx].any():
                # 优化点10: 批量处理有效模态
                text_feat = self._process_text_features(idx, B)
                mod_feat = split_x[idx]
                # print(f"mod_feat shape: {mod_feat.shape}, text_feat shape: {text_feat.shape}")
                attn_module = self.cross_attns[int(torch.log2(torch.tensor(C / 8)))]
                # print(f"attn_module: {int(torch.log2(torch.tensor(C/8)))}")
                out = attn_module(mod_feat, text_feat, text_feat)

                processed.append(out.unsqueeze(1))
            else:
                temp = torch.zeros_like(split_x[idx]).unsqueeze(1)
                # print(f"temp shape: {temp.shape}")
                processed.append(temp)

        return torch.cat(processed, dim=1).view(B, -1, H, W, Z)
...
#     def forward(self, x, mask):
#         # mask=Tensor:(1,4) tensor([[False, False, True, True]]
#         # x =Tensor:(1,4,8,128,128,128)
#         B, K, C, H, W, Z = x.size()
