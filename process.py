...
for file_name in name_list:
    ...
    flair, flair_header = medio.load(
        os.path.join(src_path, file_name, case_id + "_flair.nii.gz")
    )
    t1ce, t1ce_header = medio.load(
        os.path.join(src_path, file_name, case_id + "_t1ce.nii.gz")
    )
    t1, t1_header = medio.load(
        os.path.join(src_path, file_name, case_id + "_t1.nii.gz")
    )
    t2, t2_header = medio.load(
        os.path.join(src_path, file_name, case_id + "_t2.nii.gz")
    )
    ...
    seg, seg_header = medio.load(
        os.path.join(src_path, file_name, case_id + "_seg.nii.gz")
    )
